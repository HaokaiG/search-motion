...
the typing rate, `#cam=` the camera's, `#glow=` the bar's glow rate, `#scrim=` the bar's plate over footage and `#bez=` the camera's easing, so an edit survives a reload and works in headless renders.

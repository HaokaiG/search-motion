...
| Two box looks | The supplied `Seach Query Box_Dark Blur.svg` and `..._White.svg` | The geometry is the same in both — 1067.86×300.24 at (426.02,389.58) — so only the skin changes. **Dark blur** is `#0A0A0A` at 35% on a 57px radius with white type and a send that starts white; **white** is an opaque white box on a **69px** radius with black type, a dark mic, and a send that is `#346BF1` from the first frame, because a white pill on a white box is invisible and both the file and the clip show it already blue. A **Box** control switches them |
...
| The glow | `PromptBox_20260819.mp4`, profile out from the box edge | Reaches **38–53px** on the four sides, **18–29** at 20px out and **4–7** at 60. The build reads 43–44 / 24–26 / 2 — it was already tuned to this look against the other clip, so nothing needed changing |

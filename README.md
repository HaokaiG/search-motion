...
| Glow direction | Chroma profile perpendicular to the bar's top edge, either side | The plate's glow is **entirely outward**. Crossing inward from the stroke it reads 9, 6, 0 and stays 0 across the bar's face; outward it falls 66, 52, 42, 38, 25, 17 over ~32px. The bloom is a fat stroke, so a plain blur put half of it inside — masking it to outside the bar's own rounded rect is what the measurement asks for. The outline has since been moved to run wholly **outside** the shape rather than straddling its edge, which the mask now backs up rather than carries alone |

...
| Context above the box | Row bands and x-height, then the supplied `Common Confusion….svg` | Two lines of the previous turn, **37px** from x=258, on a pitch the plate measures at 48 and which now runs **1.4** by request — 51.8 at this size, so each row sits 3.8 further from the one below it. Written as the ratio rather than the product, since the ratio is what was asked for and it survives a change of size. **Anchored by its bottom edge** so a longer turn grows up and out of frame rather than down into the box, and that bottom is now **183** — 18 lower than the plate's 165, by request. The action sheet hangs off the same pair of custom properties, `ctx-bot + ctx-gap`, so the **55px** under it is fixed however long the line runs: nothing in act 1 carries an absolute y for either any more. Verified in an export at two lines and at four — bottom 183 and gap 55 in both, with the fourth line at y=−9, out of frame. The ink is the design file's: a group at **80%** carrying a linear fill and a layer blur the file sets to `stdDeviation="0"`, so there is nothing to add for the blur and the copy stays sharp. The fill runs (619.343,249.316) → (611.843,−114.184) over a 1433×138.4 block — 363.58 long, 2.06% off vertical — white at 50% then `#666666` at 50% at 62.706% along; resolved against that block's edges, white sits **110.89px below** the bottom and `#666` **117.04px above** it. Kept in px from the bottom, not percent, for the same reason the block hangs off its bottom. It is drawn as **regular text** carrying that ramp in a mask, by request, rather than as transparent text with the gradient clipped into it: the colour is the file's bottom tone stated outright, 181, with its two alphas — the fill's 50% under the group's 80% — folded into a single `.4`, and the mask falling to `102/180.6 = .565` by 117.04px up. A last stop takes it to nothing at 342.86, folding the dissolve into the same ramp. The two constructions are near-identical in pixels — mean stem width 6.85 with the mask against 6.87 with nothing and 7.07 at full strength, the same glyphs within a quarter pixel — so this is a change of construction, not a fix for thin type. *(A single scanline's median run says 4 against 5 and reads like thinning; it is a fixed threshold losing more of each edge on dimmer ink. Worth knowing before trusting that number.)* Exported peak ink per row, bottom up: **71, 57** at two lines and **71, 57, 43, 32** at four — the bottom row does not move when the leading changes, because the ramp's stops are px from that same bottom; only the rows above it slide further up it. The file is dimmer than the plate, which reads 120 and 86 — act 1's f1 goes 5.0 → 4.63 and the whole-piece rms 2.76 → 2.77 |

...
- **set the prompt's typing speed too** — the same dial, for the Prompt/Highlight box, in its Look group beside Glow speed: 0.2x to 5x on the slider and up to 20x in the number, the two driving each other, `#speed2=` carrying a non-default. Built the way the other piece's is rather than as a new mechanism. The plate's 37 characters cross frames 3..34, so **0.838 frames a character** is the rate that is kept and the WINDOW is what moves: a longer prompt types for longer and the typing's landing moves off the plate's frame 34 by `N2_TAILF` frames — and everything after it is quoted in plate frames against a clock that runs that many frames late (`render2`'s `fT`), so the send lighting, the press, the morph or the lift, the flash, act 2, act 3, the export seams and the piece's own end all follow without being told. Before this every prompt was crammed or stretched into the same 31 frames. Two things do NOT ride that clock, on purpose: the typing itself and its caret run on real time, because they are what shifts it; and the box's glow sweep is pinned at both ends — it opens on the plate's first frame (real) and lands at the click (plate) — so it is stretched over the real distance between the two rather than delayed, which would have started it late on a long prompt and before the piece on a short one. Verified: at 1x with the reference prompt the tail is **exactly 0**, the schedule is byte-identical to what it replaced and the piece is the plate's 8.066s; at 0.5x the last character lands on 65 against 34, act 2 arrives on real frame **86 against 55** and the piece is 9.359s (+31 frames exactly); at 2x the tail is −15.5 and act 2 arrives on 39; a 79-character prompt at 1x types to 69.19 and the piece runs 9.534s. The search piece is untouched. Named `N2_TAILF` because `N2_TAIL` was already the trailing line's text
...
- **set the AI Mode button's ring: its speed and its break** — two dials under the bar's, `Button glow speed` and `Button glow break`. Speed is a time scale on the ring's own cycle, 0-4 on the slider and up to 8 in the field; 1.00 is the look the piece ships with and 0 freezes it. Break is where the glow vanishes, in **screen degrees** — 0 right, 90 bottom, 180 left, 270 top — and it moves the ring's one existing gap rather than opening a second: the clip that cuts the gap had no `from`, so its notch was welded to the bottom, and turning the mask past it read as a new hole opening somewhere else. **The default is the right side, by request**; 90 is the way back to the bottom the source has. Measured over the full cycle against the clip fully open, every setting leaves exactly one dark side and it is the one named — the right keeps 0.19 of its glow at 0, the bottom 0.57 at 90, the left 0.09 at 180, the top 0.50 at 270 — the ends biting deeper than the long edges because a fixed wedge covers more of a short cap than of a long side. Since the ring now wraps the whole button (see the table), speed turns the colour wheel inside the ring rather than a sweeping arc. `#`-less: neither dial is carried in the hash yet
- **stop an export** — a `Cancel Export` button that appears only while one is running, since a control that does nothing is worse than no control. Every export is a frame loop that yields to the event loop once a frame, which is the only reason a button can be clicked at all mid-run — and therefore the only place a cancel can be noticed, so the loops **ask** once a frame rather than the button trying to interrupt them. All five loops (the GIF's two passes, the MOV, the MP4 and the PNG sequence) share one `bailIfCancelled()`, and all four handlers report through one `exportEnded()`, so a cancel cannot come out as "failed:" in one of them and not the others. It is thrown as a **Symbol**, not an Error, for exactly that reason: as an Error it would have been caught and reported as a failure, which is the wrong thing to tell someone who pressed the button. Nothing is written on a cancel — every format assembles its file after the last frame — and `busy`, the button and the cancel flag are one state set in one place, so a Cancel pressed too late (after the last frame, while the file is packing) cannot carry into the next run. Verified by running it: cancelled in the GIF's colour pass at `colours 27/48` and again in its writing pass at `frame 2/48`, both ending `cancelled` with no file offered and playback resumed, and a full PNG and a full GIF export both completing normally straight afterwards. It also closed a leak on the way past — the MP4's `VideoEncoder` was declared inside its `try` and closed only on the success path, so any throw at all left it open; it is hoisted and closed in the `finally` now.
- **the caption lines under the controls are off, by request** — the char/seconds line under Typing speed, the deg-per-second readouts under both glow dials, the scrim and bubble hints, and the prompt piece's char count. Hidden by one CSS rule rather than removed: the code that writes them still runs and is the record of what each control does, so deleting that rule brings any of them back. What stays visible is different in kind — the answer-overflow and lit-phrase warnings only speak when something is wrong, and the export status is the export's only progress feedback; those are not captions. Several bullets above still describe what the readouts SAY, which remains true of the hidden text and of the hash/behaviour they describe
...
| The chip is one grey with the bubble, and one blue | Exported pixels, face colour taken as the most common value per row so the ink cannot skew it | **Two departures, both by request.** The pill is `--chip2` now — the same variable `.qchip` wears — so the AI Mode chip and the chat bubble it becomes are one colour held in one place rather than two copies of a number. `CHIP_RGB` is READ off that property instead of restated in the script, because it is what the blue is mixed FROM at the press: a copy that drifted would show as a wrong starting colour part-way through the fill, which is harder to spot than a wrong resting pill. Measured, the chip at rest and the act-2 bubble both export **241,242,246**. **And the fill is one blue, not three.** The clip really does carry a shallow vertical ramp — sampled down its face it reads (51,115,244) a fifth of the way down, (64,115,240) at the middle and (88,117,209) at four fifths — but that bottom stop is 11 points greener and 35 duller in blue, and it reads as a cast rather than as shading. The face is the top sample throughout now, which is the value the reference's solid blue is quoted at everywhere else here. Measured across seven rows of an exported frame, the face is **51,115,244 at every one of them**. This is a departure from the measurement, not a correction of it. The pill went #F5F8FF (Chip.svg) to #EFF1F2 (AIM Chip.svg) to this on the way. Back: `CHIP_BLUE_RAMP` drawn as `linear-gradient(a, b 55%, c)`, and an explicit rgba in the `.aichip` rule |
...
| The opening outline is thicker | A luminance column through the bar's top edge, exported frames | **By request.** `.barfill` is the ring the bar wears before the glow arrives, and it was `2.09px` — the source's 1px on its 112-tall plate carried onto this bar's 234, x2.089. It is **3px** now, a departure from that measurement rather than a correction of it. At 1920 the CSS 2.09 was rasterising to 2 rows anyway, so the rendered change is a clean **2px to 3px of `#ebebeb`**, x1.5 rather than the x1.44 the numbers imply. Only the opening is affected even though the rule is static, and that is worth saying because it is not obvious: `.barfill` is the ONLY thing that ever draws an outline — the gradient's own padding is the bleed exactly, which takes it off the bar's edge — and its opacity is 1 only until `BAR_BURST_F0`, after which it fades on `BURST_BORDER` and never returns (measured 1 at f1, 0.675 at f20, 0 by f80). So thickening the ring and thickening it "at the beginning" are the same edit here. Back: 2.09px |
...
| The button's ring wraps the whole pill, and builds | `SearchIntro_02_V07_AB.mp4`, peak chroma round the button per frame normalised to its own top, and the lit area on the glow's first frame | **By request, and the source agrees on both counts.** The chip carried a travelling window that lit at most 57% of the perimeter, so a third of the button was always dark and the arc read as the subject. The source's glow **arrives already wrapped**: 6041 px of ring lit on its very first frame (f120) and only 8091 at its most, a 34% growth rather than a sweep opening from nothing. The chip's window is a full circle now and the only dark place is the break the dial places. And it **comes in dim and builds**: nothing through f119, then `.679 .731 .806 .813 .858 .903 .963 .993 .985 1.000` at f120-129 — two thirds on arrival, nine frames to full, a sustained climb where `RING_FADE` alone did 92% of its rise in its first 4 of 7 frames and snapped. `CHIP_RISE0 = 0.68` rides on top of that envelope over 9 frames: f115-122 go .052 .252 .377 .520 .761 .808 .867 .929 against the old .077 .352 .502 .661 .926 .942 .971 1.000, same arrival and same press-out. Measured at f122 the ring reads top 12.5, bottom 14.8, left 6.2, **right 0.9** — one opening, where the break says. Back: the old 28/44/88/99 window is written out above the rule that replaced it, and `CHIP_RISE0 = 1` |
| The button's ring, denser over footage and under a matte | Alpha the ring ADDS, isolated by differencing the transparent export against itself with the ring off; the white card as the target | **By request** — the same complaint the bar had, and the same cause: on the card the glow lies on white, in these modes on the bar's 20% black scrim, where a blurred pastel has no contrast left to spend. It measures STRONGER there (delta 10.31 against 5.98 on the card) and reads weaker, which is why this was measured through the export rather than eyeballed. Two levers are dead ends worth keeping: the bar's own remedy, extra passes, tops out at **1.09x** the added alpha for two at `BOOST_BLUR_K` and 1.27x for four at full blur, nowhere near the bar's `1-(1-a)^3`, because this glow sits over a scrim already part-way up and added alpha saturates instead of stacking; and `mix-blend-mode` screen / plus-lighter change nothing at all (mean chroma 16.1 / 15.7 / 15.8), having no backdrop to reach inside their own stacking context. What works is opacity, the lever the bar could not use because its gradient is already at 1 once the ring is round — this one runs 0.05 to 0.93 across its life. Swept against the card, 1.3 still read thin and 2.0 went neon in the matte; **`CHIP_GLOW_DENSE = 1.6`**, applied as a gamma `op^(1/k)` rather than a multiply: a straight x1.6 clamped the ring at 1.0 from f119 to f125, seven flat frames, throwing away the build just given. The gamma cannot clamp and lifts where the thinness is — **x3.04 at f115, x1.03 at the peak**, zero clamped frames, build still monotonic. Two `.boost` passes ride along for what density they do add, at FULL blur, not the bar's 0.6, which measured worse here. The card is untouched, byte for byte. Back: `CHIP_GLOW_DENSE = 1` and the two `.boost` divs out of the chip |
| The break moves; it does not multiply | Glow energy per side over the full 11-frame cycle, against the clip fully open | The vanishing point on the button was **unmovable**: `--composit-clip` had no `from`, so its transparent notch sat at 50% of the turn — screen bottom — for good, and turning `--mask-angle` moved the window PAST that fixed hole, which read as a second break opening rather than the bottom one moving. `from var(--break-angle, 0deg)` lets the notch travel; the default lives in the `var()` fallback because a property declared on `.gblur` itself would outrank the one `#chipgrad` sets and the dial would never reach the clip — which is exactly what the first version did, four settings rendering byte-identical. The bar and piece 2 never set the property and are unchanged (`from 0deg` is what a from-less conic already was). **Default right, by request**, where the source has it at the bottom |
...
| Only the chat rolls up; the chrome holds still | Per-row vertical offset written by the settle, and the rendered frames differenced once the chrome is opaque | **By request.** Seven rows shared one settle: the mark (27px), the nav (43), the rule (43), the two sidebar icons (27 each), the query bubble (44) and the answer body (48). The first five are the page the chat arrives ON rather than part of the chat, so they now travel **0** and only fade in; the roll-up belongs to the bubble and the answer alone. `data-rise` is set to 0 rather than the rows being taken out of the animation — the settle still runs on them, it simply has no distance to cover, and the opacity ramps (`data-f0` / `data-fd`) are a separate expression, so every row still arrives on exactly the frame it did: chrome f139, bubble f136, both reaching 1. Measured over f132-160 the five write 0px where they wrote 27-43, while the bubble still travels 44 and the body 48. And on the rendered frames, once the chrome is fully opaque, **f141 against f150 gives 0 differing pixels** across the nav's whole 99,840-pixel box and the rule's 20,328, while the bubble moves 20px and the body 26px over that same span. (A first check reported the nav still changing; that was the band I sampled, not the page — the bubble sits at y=288, inside what I had called the chrome band. Re-run against each row's own rect it is clean.) Back: gicon 27, nav 43, rule 43, both sideicons 27 |
...
| The bar's plus: square ends, darker ink | `SearchBar_20260819.mp4` — profiles across an arm's tip; ink sampled along an arm | **Square ends are the clip's.** Profiling ACROSS the very tip of an arm gives 254,146,73,77,82,208,255 against 254,148,76,73,86,212,254 two rows in — the same profile, which a rounded end could not give, since it would lighten at the ends of the tip row. So `border-radius` 3px -> **0**. **The colour is `#1f1f1f` by request**, which is also the ink both design files use for the AI Mode button's label and lens, so the bar's icon and that button now share one value. It went `#3c4043` -> `#999` (40% of black, asked for and then asked back) -> here, and is written as a COLOUR rather than an opacity throughout: opacity would multiply into the footage and matte looks, where this ink is deliberately switched to solid white, and would ride into the export's alpha as well. Measured back at exactly `rgb(31,31,31)` live and in an exported frame, with the footage look still `rgb(255,255,255)`. For reference the clip's own arms measure `rgb(79,79,79)`, **69% of black**, where `#1f1f1f` is 88% — darker than the clip, as `#3c4043`'s 76% was lighter. The clip's plus is also a different size, 33 square on a ~3.5 stroke against this build's 58 on 6, which was not touched. **The clip's icon also turns** a quarter clockwise as the bar arrives, and this briefly did too before being taken out again by request. The measurement is kept in the code because restoring it needs nothing else: tracked by taking ink in a ring round the icon's centroid and finding the angle whose four arms hold most of it — only meaningful mod 90 for a plus, which is what that returns — the clip gives nothing to f3, then **43** (faint), 65, 73.5, 79, 85, 86.5, 87, 88.5, settled just past upright from f19; best fit `cubic-bezier(0,0,0.2,1)` over 0..14 at **1.36 deg rms**, the house standard-decelerate over 1..15 at **1.53**. Back: `#3c4043` and `border-radius:3px` for the original pair |
| The radius blooms once per LAP, not once per burst | Blur read per frame off the live filter, against the mask's own turn | **By request** — the second rotation had no radius. `BURST_BLUR` is a lap's worth of shape: 1x, up to **20x** a sixth of the way round, down to 5x, out to 7x, back to 1x. In the source a burst WAS a lap, so reading it off `burst` was the same thing; stretching the burst to 15.6s broke that. Act 1 now carries **1.38 laps**, the second opening at **f122.6**, and on `burst` the blur had bloomed to 54.3px around f39.6 and decayed monotonically ever since — **10.2px and still falling** when the second lap began, so that lap ran with no radius at all. Driven off the lap's own fraction it reopens each time: measured, **two blooms, f33.7 and f138.3**, at 52.1 and 53.5px against the track's 54.3 peak. **The seam is free** — the track begins and ends on the same 1x, so wrapping 1 -> 0 is continuous, measured 2.88 / 2.76 / 2.82 / 3.48 / 4.79 across f122.0..123.2 with no step to hide. Every written frame matches the closed form exactly; the frames before f19 are not written at all, the gradient's opacity still being 0 there, so anything read off the element then is the previous frame's value rather than this one's. **The first lap's bloom moves** as a consequence, f39.6 -> f33.7, about six frames earlier, because a lap at 104.6 frames is shorter than the burst at 144.2 and the track's 15% point arrives sooner; its magnitude is unchanged. The chip's halo still reads `burst1x`, not this — its blur is pinned to the press and has no laps of its own. Back: `track(BURST_BLUR, burst)` |
| The chip's backdrop blur reaches the footage in the FILE | Stripe contrast (SD) under the pill against a 20px striped plate, exported frames, with the new step bypassed and not | **By request.** The pill is 75% over footage and the design defocuses what shows through — but a `backdrop-filter` can only blur what its OWN document painted behind it, and the footage is not in that document: `stageSVG` hides `bgvid`/`bgimg`, and `drawStage` composites the video underneath the SVG it then draws on top. So the exported file dimmed the plate without defocusing it. `chipBackdropInto` blurs the canvas under the pill in the one gap where the footage is down and the stage is not yet over it, reading the LIVE element for its rect, its radius and whether the filter is even on, so it cannot drift from the rule (the press turns the filter off and this goes with it) and the CSS 10px is carried onto the export's scale. Measured, stripe contrast under the pill: **21.9 -> 8.3 at f80, 22.0 -> 8.5 at f90, 17.3 -> 14.6 at f110**, against ~100 just outside it. **The preview was never broken** — that was checked first and the hypothesis was wrong. A `filter` makes an element a backdrop root, `#world` carries `url(#mblur)` exactly while the bar moves, and the chip lives inside it — so the blur should have died during the pan. A controlled A/B with motion blur on and off says otherwise: the frames are the same and the stripes are smoothed in both. (The first attempt at that A/B compared a frame where the video had not decoded yet against one where it had, which is not an A/B at all; a striped STILL removed the decode timing.) **Blurred against the whole frame, not a crop.** A padded crop was tried first: canvas `filter` treats everything outside the drawn image as transparent black, so the blur pulls it in at the patch border, and where the pill runs past the frame edge the padding is clamped — measured on f70, where the chip runs 1558..1991 of 1920, the crop version made contrast **WORSE than no blur at all, 64.6 against 54.2**. Against the whole frame that frame reads 16.7. Cost: **+13.4ms a frame** on a 1920 export, 34.6 -> 48.0. Untouched, verified byte-identical with the step bypassed: the transparent export (`keepAlpha` skips it — an alpha file has no backdrop to blur, so this is not expressible there and nothing pretends otherwise) and the plain white card (no footage). Residual: on the fastest pan frames the SVG's pill is motion-smeared while the blurred patch is at its sharp rect, so f75 only reaches 30.5 -> 22.3. Back: drop the `chipBackdropInto` call |
| The transition waits for the click to finish | The rendered scaleX for the click's end; the first frame the face, the wash and the rect each move; the box's landing against the chip | **By request, and it restores the piece's original intent rather than inventing one.** `N2_SPLIT_FP`'s note has always called this frame "when the button click motion is finished", and while the press was its measured 350ms it ended on 48.994 with the rect opening on 49. At 378ms that frame is **49.67**, and `N2_TRANS_OPEN` is derived from it — which is why the click going 10% quicker afterwards moved the whole transition on its own, from one constant. It opened on the stretch's PEAK for a round before this. **It moves whole this time.** The peak version shifted only the opening beats and left the landings, stretching the transition; from the click's end the landing at 52 would have left barely a frame for the entire morph. So the landings move by the same shift — `N2_MORPH1`, `N2_FLASH1`, `N2_A2`, `N2_SETTLE_END` and act 2's own nav ramp — and the transition keeps the shape it was measured with, 2.67 frames later. Act 3 does NOT move: `N2_A3`, the exit smear and the 79.3 fade are its own timing and the piece keeps its 8.066s, so act 2 simply holds 2.67 frames less. Measured at 378ms: the face first moves on 49.75, the wash on 50.75, the rect on 51.75, act 2 arrives on 53.67 and the nav still comes in about **3 frames after it**, as it always had. **The glow and the furniture needed no redoing** — they were already derived from the transition rather than baked, so they moved with it: the halo, the send, the mic and the plus all begin leaving on 49.75 and are gone together on 51.75. That is the whole value of having un-baked them. **And the seam means what it says again.** `N2_SPLIT_FP` was pinned at 49 while the transition ran early off the peak, because following the wash would have cut the prompt export mid-click; with the transition back on the click's end the seam IS the transition's opening, the coincidence the piece was built on, so it is derived again — 49.67 against 49, and the prompt half is 0.67 frames longer. One consequence caught and fixed: `N2_SETTLE_END` now adds `N2_SETTLE_EASE / 2`, because the ease delays the settle by that much and the piece's measured 12.2-frame window would otherwise hand the chip a box **1.7px** short of it against the bare exponential's 1.3px. With the ease absorbed it lands **1.56px** off, as close as it ever was, and the tail still reads 0.7098-0.7101 against the fitted 0.71. Back: `N2_TRANS_OPEN = 48` for the piece's own numbers, or `N2_PEAK_F` for the stretch's peak |
| The click's length, and the transition that follows it | The rendered scaleX walked at 0.02-frame steps; the first frame each of the face, the wash and the rect actually moves | **By request, over three rounds.** `N2_PRESS_MS` has been 350 measured, then x1.1, then x1.2, and is now **10% quicker than that x1.2 — 420 x 0.9 = 378, which is exactly 350 x 1.08**, so the request reads against what was on screen while the constant still reads against the clip. The multiplier is always written against the measured 350 rather than compounded, so it says how far the piece has departed at a glance. The down phase moves in proportion: **1.930 frames to 2.084**, peak on f42.69 against 42.53, back at rest on **f49.67** against 48.994. Measured peak on the rendered transform: **f42.64 at scaleX 1.25**. **And the transition now opens on that peak** rather than on 48, which was after the click had already started back. `N2_PEAK_F` is derived from the press, so lengthening the click carries the transition with it instead of letting the two drift; `N2_TRANS_SHIFT` moves everything that OPENS the transition by one amount so their spacing survives — `N2_MORPH0` 48 -> **42.92**, `N2_MOVE0` and `N2_FLASH0` 49 -> **43.92**, `N2_SETTLE_F0` 49.8 -> **44.72** — while everything it LANDS on is left alone: `N2_MORPH1`, `N2_FLASH1`, `N2_A2` and `N2_SETTLE_END` all stay. The transition is stretched, not moved, so nothing downstream is re-timed. Verified: the face first changes on f43, the wash on f44 and the rect on f44.75, against those three constants; and act 2 still arrives on 52.25, act 3 on 87, the box still goes on 62, the piece is still 8.066s and the `all` / `prompt` / `response` export seams are unchanged at 194 / 50.4 / 194. The lift reproduces its measured table exactly from its new start (0, 1.81, 8.68, 22.49, 46.89, 86.55) and cuts on 48.92. `Prompt + Lift Out` follows the cut, 57.4 -> 52.32. **`N2_SPLIT_FP` is pinned at 49 now** rather than written as `N2_FLASH0`. It was the wash's frame because three things agreed there — the click's end, the rect's morph and the wash — and its own note calls it "when the button click motion is finished". None of the three lands on 49 any more (the click ends on 50.67, the wash opens on 43.92), and following the wash would have dragged the seam five frames earlier and cut the prompt export in the middle of its own click. The seam keeps its frame and the coincidence is over. Back: `N2_TRANS_OPEN = 48` restores 48 / 49 / 49 / 49.8, and 350 restores the click |
| The morph empties the box in one move, and opens eased | Per-frame opacity of each piece of furniture; the rect's per-frame share of its own travel | **Two, by request.** **One, the glow leaves with everything else.** It faded on a baked `1 - (f - 48.3) / 1.9` — 0.3 frames after the morph opened, over 1.9 — which was true while the morph opened on 48 and stopped meaning anything once it moved to the stretch's peak: the send, mic and plus were leaving from 42.92 and gone by 47.46 while the halo did not start until 48.3 and hung on to 50.2, about three frames after the box had been emptied. All four now share one `leave`, and measured they reach zero on the same frame, **47.5**, tracking together the whole way down (glow and send identical; mic and plus at half, which is their resting `dim`). **Two, the rect opens eased.** It rides an exponential settling toward the chip, and an exponential is at its fastest on its FIRST frame — measured, 0.290 of the whole distance in frame one against 0.206 in the next — which put its hardest movement exactly where the button is still contracting out of its stretch. The ease is on the CLOCK, not the curve: the exponential reads an elapsed time that grows quadratically from a standing start over `N2_SETTLE_EASE` frames and then matches `e - EASE/2`, so the measured tail is preserved exactly and only the opening is shaped. Verified — the first frame now moves **0.102** against 0.290, the hardest frame is the second rather than the first, and the tail's per-frame ratio still reads **0.7097-0.7101** against the fitted 0.71. The whole thing lands EASE/2 = 0.8 frames later, which the settle's window absorbs: the box still arrives on the chip at top 291.4 against 291 and width 682.4 against 681. The lift is untouched (it zeroes the settle). Back: `N2_SETTLE_EASE = 0`, and `sat(1 - (f - 48.3) / 1.9)` for the halo — though that wants re-basing onto `N2_MORPH0` rather than restoring as an absolute |
| The click plays 10% longer | The rendered scaleX walked at 1/8-frame steps, against the same two-segment curve evaluated at both durations | **By request.** `N2_PRESS_MS` 350 -> **385**, x1.1. The whole press is stretched, so the down phase — the button widening to `BTN_SX` 1.25 over the first `PRESS_PEAK` of the run — goes with it in the same proportion: **1.930 frames to 2.123, exactly x1.1**, with the peak landing on f42.68 instead of f42.50 and the button back at rest on f49.83 instead of f48.994. The rendered transform matches the model to **5e-5** across the whole press. **What it costs**, since 48.994 was load-bearing: it is why three things sit on 49 — the seam between the halves, the rect's morph (`N2_MOVE0`) and the flash — and `N2_SPLIT_FP`'s own note calls that "when the button click motion is finished". The click now ends 0.84 frames after them, so the rect starts travelling while the send is still returning. Nothing else was moved, so it is a real overlap rather than a re-timing — but a sub-pixel one throughout: measured against the old curve the pill is **0.70px wider at f48, 0.19px at f49, 0.03px at f49.5** on its 94.29px width. The energy blob keeps its own measured 400ms. Back: 350, or move `N2_PRESS` earlier by the 0.84 if the overlap ever needs to be clean rather than merely invisible |
| The prompt box takes the bar's glow construction | Layers rebuilt on the box's own scale; coloured area measured in a band round it, A/B against the version it replaced | **By request** — the same travelling arc the search bar carries, on this piece's box. It was a full ring: a single 17px bloom on the box's rect with a wheel rotating its hues inside, lit the whole way round and never dark anywhere. Now it is the bar's stack — the source's gradient under the same pair of conic masks, cut to the ring, grown past the box so the blur's spill survives the cut — with the **radius blooming once per lap** off the same track. **Scaled by its own height**, the rule the bar and the chip use: `K_BOX` = 300.234/112 x 1.3 = **3.485**, where the bar is 2.716 and the chip 1.834 — this box is the largest of the three. **The wheel stays this piece's own**, `N2_WHEEL` with its by-request fade on the blue side: the motion was what was asked to match, not the colour. **It runs BOTTOM-LEFT corner to BOTTOM-RIGHT corner, clockwise, by request** — one journey up the left, across the top and down the right, with this piece's own measured motion set aside (`N2_RING_F0` and the 1.149 deg/frame fitted off its clip are kept only as the record). **Two structural finds made the fit possible.** First, the source's `--composit-clip` is a fixed notch — it has no `from`, so it never turns — and against this element's 92%-wide window it pinned the visible gap at perimeter fraction 0.945..0.315 at EVERY mask angle from 0 to 345; it is disabled here (overridden to solid black, structure intact). Second, `N2_WHEEL` — this piece's wheel, with its blue side alpha-faded — was shaping the band from inside: the faded stops read as HOLES in the glow, a blue patch stranded on each side of one, which is the "gaps between gradient glow" that got it replaced. **The band now shows the SOURCE's wheel**, the 16-stop conic the bar carries, which begins and ends on `#3186ff` so blue connects to blue through one continuous run; `N2_WHEEL` stays defined as the record. The wheel and the window turn TOGETHER, one turn with a fixed offset between them: a point at window coordinate t sits at wheel coordinate t - OFF, so with the band spanning 28..99% and the spectrum at 34..73%, **OFF = +36deg** maps the band onto wheel 18..89 — both ends inside the blue that runs 73..134 through the wrap, the spectrum whole in the middle. Measured: **one gap and only one at every frame sampled, both band ends on hue 210-218**. (-45 was tried first from the same stops run backwards, and measured one end on red.) **The endpoints are fitted to the corners by measurement, twice** — the corners sit at perimeter fractions 0.890 and 0.500, and each wheel change moves where the band's centroid sits for the same angle, so the pair went 312/220 (faded wheel, static), 307/233 (walked live), then with the source wheel 13/230 static and **356 with a 257-degree clockwise turn** walked live. Measured on the final pair: **f1 at 0.881 and f49 at 0.500 exactly**, monotonically clockwise between them. Near a corner the response is nonlinear — ~0.0053 fraction/degree, double the perimeter average, the band's mass tipping between two edges — which is why every static fit needed a live walk after it. The travel never completes a lap, so **the blur rides the sweep's own progress** — the source's tie: one burst, one bloom, back to 1x by the click. **The radius runs SMALL to NORMAL to SMALL, by request, in two steps**: first the bar's 20x opening bloom was trimmed to 8x (it hit **69.7px on this element** around f8 against 17-24 through the act's middle), then the residual 8 -> 5 -> 7 wobble went too — one level was what was asked. The box's track now rises from 1x to a **6x plateau** over the sweep's first fifth, holds it flat, and falls back to 1x at the end; 6x is where the old track's own middle sat, so the plateau is the "normal" the piece already had, minus the drift. Measured: 3.5 rising to **20.91 by f10, dead flat through f37** — the same number at every sample between — then down to 3.5 at the click, with **zero mid-act dips**. **The sweep has piece 1's own pair of dials, by request**, replacing the Glow Motion 0.5x/1x/2x dropdown: a Glow speed slider (0-4, field to 20, `#glow2=`) and a Glow Easing chart (`#g2bez=`), the same `makeBez` editor instance. Speed is a time scale on the sweep's own clock, **rebased by request so the dial's 1.00 IS the old 0.35** — `N2_GLOW_BASE` folds it in, so the scale renumbered rather than the default moving, and a `#glow2=` saved before the rebase now reads 2.86x slower than it did. At the new default the sweep reaches **90 of its 257 degrees by the click** (measured 89.9, partway along the top) and no longer finishes the corner-to-corner journey; **2.86 on the dial is the old full journey**, measured back at exactly 257 by the click. 0 freezes it on the bottom-left corner. **The radius envelope now rides act 1's own clock, not the speeded sweep** — at the rebased default the speeded clock only reaches 0.35, so the blur would have held its plateau into the morph and the small-to-normal-to-SMALL just asked for would never have ended small; on the act's clock it completes at the click at every speed, measured 3.48 / 20.91 / 3.48 at f1/f20/f49 with the dial at 1 and the same 3.48 close at 4x. The curve shapes **only the angle**, the rule the bar's easing follows; its default is linear, which is load-bearing — on the standard decelerate the sweep passes the bloom by about f2, with the box barely on screen. **The dial drives the QUERY BOX's sweep only**: the highlight scene's glow, which shared the old dropdown, keeps its measured rate — verified identical at 0.5x and 2x. **The wheel's `from` stays fixed** at the measured hue phase; the element does all the turning. **The window is fitted to a supplied still** of how the glow should sit round the box — lit across the top and down most of both sides, the bottom edge clear. The target is **51%**, worked out from the picture rather than eyeballed: on a box this wide the top and bottom edges are 39% of the perimeter EACH and the sides only 11%, so "the top and most of both sides" is 39 + 7.7 + 4.4. By eye the image reads like two thirds of the way round; the arithmetic says half, and the arithmetic is what the mask has to hit. Measured at five frames across the act, the window walks the lit share up — 28/44/88/99 gives 33%, 18/32/92/99 gives 40, 12/24/94/99 gives 44, 8/18/96/100 gives 48 — and **5/12/97/100 gives 50**, measuring back at a **mean of 52% against the target's 51**, range 44-62. It plateaus there because `--composit-clip` takes its own bite out of the arc however far this opens. That also fixed what the source's window did on this box: at ~39% of the circle there were frames with **261 coloured pixels** against the old ring's steady 41-48 thousand, a dark frame in all but name. **What it costs, measured A/B**: the arc averages **28,153 coloured px a frame** against the old ring's flat **41-48,000**, peaking at 90,837 during the bloom and dipping to 10,000 — it breathes where the ring sat still, which is what an arc does and what was asked for. Two corrections on the way: everything was first scaled off `#n2box`, which is a PANEL control and not the stage's box — the real element is `#nbox` at 1067.86 x 300.234, aspect 3.56, radius 57, not the 1259 x 143.6 at aspect 8.77 the wrong one reported — and the squash went 5.6 -> **2.6** with it, this box sitting nearer the chip's 2.88 than the bar's 10.9. A CSS rule injected into `<head>` also will not reach an export, since `stageSVG` serialises the original style tag; anything being fitted has to be written into the file. Back: the `.nglow b` bloom at `blur(17px)` with `.nglow i` at 1067.86 square and `scale(-1,0.436)` |
| A second way out of the prompt: the LIFT | `PromptBox_20260819.mp4`, sub-pixel top edge of the box per frame, cross-correlation of the context paragraph against it, and top-of-frame luminance | **By request, as a choosable alternative** to the morph — the box no longer shrinks onto the chip; it and the page's furniture travel straight up and out while the chat rolls up on its own. `Transition: Morph / Lift` in the piece-2 group, **Morph is still the default**, and `#n2t=lift` carries it. The clip opens on the same beat the morph does — the box is dead still through its f48 and moves from f49, and `N2_MORPH0` is 48 — so the two are alternatives at one frame rather than two different edits. Risen from rest, f48..f53: **0, 1.81, 8.68, 22.49, 46.89, 86.55**, and the render reproduces all six exactly. It is **one translation of the whole page**, not a per-element stagger: cross-correlating the context paragraph gives -2.00 / -9.00 / -22.52 / -45.77 against the box's -1.83 / -8.62 / -22.00 / -45.32, equal within the measurement. No single curve fits — the power reproducing each successive pair climbs 2.26, 2.35, 2.55, 2.75 — so it is quoted as measured and interpolated between. **It is CUT OFF, not flown out**, by request and on a second look: f53 is the last frame with any picture and the box is still COMPLETELY on screen there — top 303 against a height of 298, bottom 601 of 1080 — at full brightness, whole-frame luminance 39.5 against 39.8 the frame before, and f54 reads 0.0. A hard cut, no fade and no travel off the top: the whole move is 86.55px over five frames and the shot ends. So the table is held past its end rather than extrapolated, and the furniture goes on `N2_LIFT_CUT` (54). The first version extrapolated at the last frame's ratio to about f56.6, inventing **eleven times** the travel the clip shows. **The ground does not move**: the reference's top 20 rows read luminance 0.0 on every frame f48-f53, so the dark page stays put and the content slides off it. `#nscrim` is inside `#n1`, so it is counter-translated against `#n1`'s travel and keeps the visibility schedule it always had. **The chat arrives the way the search piece's act 2 does** — bubble and response rolling up together — but on THIS piece's measured curve, `N2_RISE_K`, not that piece's numbers; the chip rides the body's climb at the ratio piece 1 holds between the two, 44/48. The chrome stays fixed. **The whole act waits for the cut on the lift**, by request — the arrival, not just the climb. Delaying the roll-up alone was tried first and recalled: it left the chat on screen but parked, which is not the same thing. It is ONE shift on act 2's own clock (`A2_SHIFT = N2_LIFT_CUT - N2_A2`, two frames) rather than a delay bolted onto each beat, because the act has four quoted against the frame — the arrival, the body's two-frame fade, the roll-up and the nav's ramp — and moving one without the others just re-cuts the act internally. The EXIT is deliberately not shifted: `N2_A3` and the body's 79.3 fade belong to act 3, so the act arrives two frames later and is two frames shorter rather than pushing the piece along. Measured: the chat is first visible on **f54 against the morph's f53**, its fade runs 0 / 0.5 / 1 on f54-56, the roll-up starts from full 397px at f54, and the exit still begins on f80 in both. Back: `A2_SHIFT = 0`. **The GROUND waits too**, by request. The white wash is measured at f49..52, which on the lift had the page most of the way to white while the box was still travelling over it — the reference does the opposite, its top 20 rows reading luminance 0.0 on every frame to the cut, and it is why the Dark Blur box looked muddy mid-flight. It is shifted so the wash STARTS on `N2_LIFT_CUT` rather than merely being delayed with act 2: the ask is that it not turn white until the box has gone, and act 2's own two-frame shift would still have had it part-way there. Its length is untouched; `N2_SPLIT_FP` deliberately stays on `N2_FLASH0`, being the structural seam between the halves rather than a beat of the wash. Measured: the page is first non-zero on **f55 against the morph's f50** and fully white on **f57 against f52**. That exposed one thing the wash had always hidden: `.n2fade`, the white gradient that blends the answer's bottom into the page, was a CSS constant — invisible while the wash always finished before act 2 arrived, and a solid white band across the bottom of a dark frame once it did not. Its opacity is derived from the wash it serves rather than given a ramp of its own, so the two cannot drift; the morph is untouched, reading 1 on all 34 frames of its act 2 as before. **The nav's rule goes the same way**, by request — it is drawn ON the page rather than being ink arriving over it, so it cannot be more present than the page is, the same construction `data-on-card` gives the other piece's rule. And it needed the other piece's SECOND correction with it: `.n2rule` was an opaque `#d5d5d8`, and a light bar faded in over a page that is not white yet can only ADD brightness — composited on a black ground it reads **127.6 against a page of 84.9** at a third of the wash and **198.7 against 170.1** at two thirds, lighter than the page it divides, which is the inverse of a divider. It is a darkening now and holds the relationship at every stage: measured off the exported pixels, **80/85, 151/170, 213/255**. The settled look is untouched — solving `a*c + (1-a)*255` for `#d5d5d8` gives alpha 0.1647 with a colour of (0,0,18), the blue carrying the grey's slight cool cast that a flat black would have neutralised (the other piece's rule is flat black because ITS target, `#ebebeb`, is neutral), and the export reads back **(213,213,216)** exactly. The morph is untouched: opacity 1 on all 34 frames of its act 2. **The bubble goes with them too**, by request, so the four things drawn on the page — the ground, the rule, the fade band and the chip — now share one number: 0 / 0.333 / 0.667 / 1 on f54-57. Its settled fill reads back **241.9**, `#f0f1f4` as before, and the morph is untouched (1 on every frame after `N2_SETTLE_END`). **It is deliberately NOT given the rule's darkening**, and that is a departure worth stating rather than an oversight. The bubble inverts the same way for the same two frames — measured off the export it reads **136.3 against a page of 85** and **216.5 against 170**, lighter than the page where it settles darker (241.9 against 255). But a divider's whole job is to be a subtle line, so the inversion destroys it, whereas a bubble's job is to be a visible container: as a darkening it would sit **1.7 levels** from the page at a third of the wash and 6.7 at two thirds, which is no pill at all — the text would read as floating. Opaque, its text carries contrast 70.3 and 139.5 on those frames. Two frames at 24fps is 83ms. Back: `background:rgba(0,0,0,.0588)` on `.n2chip` would hold the relationship at the cost of the shape. **The prompt half can carry the lift too**, by request: a fourth Export option, `Prompt + Lift Out`, which runs the same half but keeps going through the lift and ends blank. Where it stops is measured, not chosen — the clip is 56 frames, its last picture is f53 and f54/f55 read luminance 0.0, so it carries TWO black frames after the cut and this half carries the same two, ending on plate 56 against plain Prompt Only's 49 (2.394s against 2.102s). Measured through the export, f54 and f55 come back **max luminance 0 across the whole frame** — nothing drawn at all, which on this piece is black in a normal export and transparent in an alpha one, since `#stage2` is transparent and the ground is the export's own fill. The option carries the lift ITSELF rather than deferring to the Transition toggle, so the two cannot contradict each other: that toggle says how act 1 hands over to act 2, and this half has no act 2 to hand to. Two derived flags now answer what six call sites used to ask inline as `SEG2 !== "prompt"` — `PROMPT_HALF` (either prompt option) and `liftHere` (does the lift run on this frame). Three things had to be gated that the plain half never reached inside its own length: the wash (it runs 49..52 and this half runs to 56, so the page would have turned white behind a lift meant to finish blank), act 2's arrival, and the scrim, which is the ground and now goes with the cut rather than with an act 2 that never comes. Verified: plain `Prompt Only` is identical with the Transition toggle either way and its box never moves, and the whole piece still round-trips at 0 differences. Back: drop the option and `N2_PROMPTLIFT_SEG`; drop `flashFrom`, `.n2fade` / `.n2rule` / `.n2chip` lose their inline opacity, and `.n2rule` returns to `background:#d5d5d8`. **Four gates, not one.** `mt` is the morph's progress but the rect has two more curves of its own (`ru`, the settle, and `u`, the power curve the font and face read) and neither goes through it — ungated, the box shrank while it flew. The chip's transform and opacity are written only inside `if (inA2)` and so survived every frame outside act 2, which left the two transitions holding different state on f44-52; both are cleared out there now and the morph round-trips through a switch with **0 differences across 23 frames x 9 properties**. **What the clip cannot say**: it cuts to black on f54, so there is no frame showing whether anything fades on the way out, or what the ground does afterwards. Nothing fades here — the travel alone takes it off — and act 2's white page keeps the schedule it already had, which means the box flies over a page that is turning white under it. Note the clip is the **white** box look, not the Dark Blur default. Back: `Transition: Morph`, and `N2_CHIP_RISE_K` / `N2_LIFT` are the two constants |
...
- The AI Mode chip's backdrop blur comes off at the press. Over footage or under a matte the pale pill is 75% there and carries `backdrop-filter:blur(10px)` so what shows through is defocused rather than dimmed; from the press it is solid blue and the blur can only cost. Profiled at 60Hz over full loops with the pane visible (rAF is throttled to about a frame a second whenever the pane is not the displayed tab, which reads as lag and is not the page), plain mode held **60fps with a 17.8ms worst frame** in every configuration, motion blur on or off, at both a 0.21 and a 0.59 stage scale — and the ONLY stalls sat in the 5.5s bin, the press-and-zoom: **367ms** with footage, **133ms** under a matte, and **17.7ms** with that one filter disabled and nothing else changed. A backdrop-filter re-reads and re-blurs what is behind its element every frame it changes, and here the element scales to fill the stage while act 2 appears behind it. The filter is now written inline per frame: `none` from the first pressed frame or any zoom, `""` otherwise so the rule keeps the pale pill's blur to the last frame before the blue — measured `blur(10px)` at f120 and `none` at f130. After: footage **17.7 / 33.6ms** worst over two loops, transparent **17.6**, the prompt piece **17.7** on both of two consecutive loops (its one 183ms frame was the first play after a reload). The export never carried this blur over footage in the first place, so it is unchanged.

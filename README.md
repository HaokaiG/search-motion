...
| Context above the box | Row bands and x-height | Two lines of the previous turn, **37px** on a 48px pitch from x=258, fading upward — the lower line's ink peaks at 120 and the upper at 86. **By request it is anchored by its bottom edge**, so a longer turn grows up and out of frame rather than down into the box: the bottom stays at 165, which is `bottom:915` on the 1080 frame, and the stage's own overflow does the leaving. The fade had to stop spanning the element with it — over its own height it always put the *top* line at .72 whatever the count, so a four-line turn would have left frame at 72% and been cut hard. It is a fixed ramp off the bottom at the slope those two lines imply — .72 at 96px up, so zero at 342.86 — identical while there are two lines, and each further line recedes past them. Measured in an export: 119, 102, 85, 68 down the four bands, and the box's top at −27 |
...
| Act-3 wall | Edge rise on a stem, and banded coverage | The body is thrown out of focus — a stem rises over **55px** there against the lit line's 13. 24.1% of the frame carries wall at a mean of 24.1. **The unlit copy is white ink at low alpha rather than the flat `#252525` it was**, by request, and it carries **two** alphas because one cannot do both jobs. Plain, it is `.145` — 37/255 — so it composites to exactly the 37 the measurement asks for and the frame means do not move: 11.46, 15.44, 10.68, 14.60, 14.12 on f87/95/124/160/192, whole-piece rms still 1.92, matte peak 23. Over footage that same value reads as gone: the old grey worked there by *covering* (an opaque 37 against footage at 100 is 63 levels of darkening) where white at .145 lifts it by only 22. So `#stage2.has-bg` / `.has-alpha` take it to **`.4`** — .4 × 155 is 62 against the old 63 — the same split the search piece already makes for its query, caret and chip. Measured over a three-band stand-in plate at rows on ground 20: peak deviation **59** at .4, against 21 at .145 and 11 for the old grey, so over dark footage it now reads better than it ever did. Over *bright* footage white ink is inherently weak — 8 levels at .4 where the dark grey gave 61 — which is the cost of the colour, not of the alpha. Back: one `.145` for both, or `#252525` for neither |

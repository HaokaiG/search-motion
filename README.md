...
| The bar goes dark over footage | Composite ratio, inside the bar against just outside it | **By request**, the plate switches to a dark scrim the moment Transparent is on or footage is loaded, at **25%** — `--bar-scrim` 0.20 -> 0.25. The rule for this already existed and had stopped working: rebuilding the plate off the source made `.barover` the FACE — the overlay inset by the border width that confines the gradient to the ring — and it carried an opaque white, so the scrim on `.bar` underneath was painted and then covered. Over footage the bar was still a white card. The scrim goes on the overlay now, `.bar` is left clear so the two cannot stack into 44%, and `.barfill` (the border colour and its shadow) goes transparent with it, which is what the old rule's `border-color:transparent;box-shadow:none` did before the border became its own layer. Measured back: inside/outside reads **0.749** across three columns against the 0.75 the scrim predicts, and the interior picks up **no** colour from the gradient (saturation 0) — the overlay still hides it. Plain mode is untouched, face and bar both #fff. Back: 0.20, and the single `#stage.has-bg .bar` rule |
...
| The button's halo: bottom, one full turn, then gone | Phase swept against bottom coverage; arc centre and halo contribution tracked per frame | **By request**, over two passes. It gets its own phase and cycle rather than sharing the bar's wheel, which had put its arc wherever the bar's happened to be. **285deg is fitted** — sweeping the mask angle against the share of the bottom run that lights, coverage peaks there. Increasing the angle moves the arc CLOCKWISE, measured: its centre runs +42 -> +49 -> +60 -> +74 -> +89 samples of 180 as the angle goes 0 -> 100. **It needed its own squash to reach the bottom at all.** The shared `scale: 7 1.5` is fitted to the BAR's aspect, 10.9; the chip's is 2.88, and the same stretch mapped the mask's window onto 27 samples against the 49 the bottom needs — no phase could have covered it. Swept, **2.2** is the best this mask shape reaches: 80%. **The turn runs 119 -> 127, both ends chosen from the halo's own brightness.** Its opacity ramps 0.08/0.35/0.50/0.66/0.93 across 115..119, so below 119 the arc HOLDS on the bottom while it fades up. And 127 rather than act 1's end: spread to 136 the turn only got about a third of the way round before the halo dimmed — it reached the top and stopped, which is exactly what it looked like. Fitted into the bright window it is 45 deg a frame. **And the halo now leaves.** `RING_PRESS_FADE` was FPS, a second, which cannot finish inside act 1 — it was still at 0.13 at the cut, so a faint outline sat on the button through the whole blow-up. **8 frames** puts it at zero by 135, one frame before the cut. Measured back: mask **285.0** at f119 with **78%** of the bottom lit at 0.93 opacity; **645.0 = 285 + 360** at f127, back on the bottom; arc centre monotonic clockwise; and the halo's contribution to the frame is **exactly 0** from f129 on — the saturation there is the chip's own blue fill and the blow-up, not the glow. Back: CHIP_CYCLE_F1 = SPLIT_FP and RING_PRESS_FADE = FPS |

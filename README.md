...
| Context above the box | Row bands and x-height, then the supplied `Common Confusion….svg` | Two lines of the previous turn, **37px** from x=258, on a pitch the plate measures at 48 and which now runs **1.4** by request — 51.8 at this size, so each row sits 3.8 further from the one below it. Written as the ratio rather than the product, since the ratio is what was asked for and it survives a change of size. **Anchored by its bottom edge** so a longer turn grows up and out of frame rather than down into the box, and that bottom is now **183** — 18 lower than the plate's 165, by request. The action sheet hangs off the same pair of custom properties, `ctx-bot + ctx-gap`, so the **55px** under it is fixed however long the line runs: nothing in act 1 carries an absolute y for either any more. Verified in an export at two lines and at four — bottom 183 and gap 55 in both, with the fourth line at y=−9, out of frame. The ink is the design file's, reduced to what survives a **white** line with a **60%** ceiling: its fill runs (619.343,249.316) → (611.843,−114.184) over a 1433×138.4 block — white@50% then `#666666`@50% at 62.706% along — which resolves to 180.6 ink at the bottom row and 102 at 117.04 up, a ratio of **.565**. That ratio is the ramp, which then **stops dead at 155.40 — three rows, 3 × 51.8** — so a fourth line and anything above it is not drawn, by request. A hard edge rather than a fade, since hiding is what was asked for, and it costs nothing below it: measured at four lines, row 3's ink tops out 147 up from the bottom and row 4's begins at 162, so the cut sits in the 15px gap between them with 8px of clearance either side. No count-based rule was needed — the ramp is px from the bottom and the block hangs off the bottom, so a cut at three rows never reaches a block that has three or fewer. Verified in an export: 2 laid-out lines draw 2 bands, 3 draw 3, and **4 and 6 both draw the same 3**, topmost ink at y≈35 against a cut at y=27.6. The file's **layer blur** is declared at `stdDeviation="0"`, and it is asked for here and **heavier toward the top**. A CSS blur is uniform, so the line is drawn twice and cross-faded — a **near** copy that is all the ink at the bottom and none by 117.04 up, and a **far** complement — so each row is softer than the one below it: weighting the two masks at each row's centre gives an effective **2.31 / 3.70 / 5.0** px going up. **Neither copy is sharp**, which the supplied crop forced: the near layer had been at blur 0, and measured on ink spread — area over a tenth of peak against area over half — the bottom row came out at **1.34** against an unblurred reference's **1.33**. Dead sharp, where nothing in the crop is. Scaling that crop by its line pitch (245px against the design's 46.14) puts it at 5.31×, and its stroke edges at ~4.1px on the bottom row and ~7.2 on the top — about 1.6 and 2.8 sigma, both upper bounds since an upscaled screenshot is softer than its source. So the near layer takes **1.6** (bottom-row spread 1.34 → **2.35**) and the far one keeps **5** rather than the crop's 2.8, the gradation having been asked to be obvious. The cost is peak: a spread stroke reaches less, so the bottom row reads 115 where the sharp one read 143 — the 60% is the layer's opacity and still is. The two masks are the product of fill and crossfade and sum to the fill; `.45` rather than `.3915` at the halfway stop corrects for layers compositing as `1−(1−a)(1−b)` rather than `a+b`. It is drawn with **greyscale antialiasing**, and that is the one that made it finally read Regular. The weight was never wrong — the axis default is 400, the advance measures 971.02 whether `wght` is pinned or left alone, and rasterised coverage matches a 400 probe — but subpixel smoothing fattens light ink on a dark ground, and in an export greyscale takes the **mean stem from 6.63 to 4.69** and 21% of the ink out at an unchanged peak (143 against 144). It thins the strokes back to the outline rather than dimming them. For scale, dropping the weight to 350 removed only 7%: the rasteriser was worth more than a whole step of weight, so lowering the weight would have been correcting the wrong number. Scoped to this line; the rest of the piece's white-on-dark type is still on the default, having been measured against the plate with it. Verified in an export: peak ink per row bottom-up **143, 84**, monotonic, nothing above the 153 that 60% allows, and the 10–90 edge rise **1px on the bottom row against 6px on the top** — the blur graduated, not uniform. Act 1 lands closer for it, 5.09 / 5.76 / 5.87 on f1/25/40 against the plate's 5.0 / 5.2 / 5.5, and the piece goes 2.77 → **2.76** rms |

...
| Both boxes at 35% | By request. The dark look already carried it; the white one was opaque, and at 35% over nothing it renders as mid-grey (255 x .35 = 89) with its black type sitting at low contrast on it. Over footage — which is what a translucent box is for — it behaves as intended. Back: `background:#fff` and `FA0 = boxWhite ? 1 : 0.35`. |

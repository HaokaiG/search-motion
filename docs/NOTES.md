...
| The camera follows the caret | **The baked `PAN` curve no longer drives the camera at all.** The pan chases a target derived from the newest character by a fixed fraction of the remaining distance each frame — a first-order lag, so it is smooth by construction and settles rather than stopping. `CAM_SPEED` scales that fraction and is exposed in the panel and as `#cam=`. The offset makes the landing exact: the target is caret − (`PARK_X` − the chip's run), so the last character puts it on `PAN_END`. `CAM_LEAD = 2.75` is why the default alpha is 0.09 and not the plate's own 0.05 — a lag always trails, and following at 0.05 with no lead leaves the pan **230px short** when the outline lights, so the bar never parks. Aiming ahead of the caret fixes both the arrival and the peak. Measured against the source clip's own camera (peak 54 px/frame around frame 65, decaying 0.95/frame): the default gives **peak 53, 1px short at the outline, fully parked by the cut**, at a decay of 0.91. `PAN`/`PAN_REF` are kept as the record and the way back. |

...
| The bar's G mark is 8.72% over the plate's, and evenly inset | Two departures on the same element, both by request. The wide shot gives 124.8 tall at our scale; the mark now runs 8.72% larger at 132.53 x 135.68, taken about its own centre so it grew in place, and it holds the artwork's 250x256 aspect (0.9768 either way). It is then squared up in the 245.44 bar the G wears: 54.88 of margin on the top, the bottom AND the left, where the plate's own left inset is 58.1 and deliberately ~6% wider than its vertical. Read off computed style the three come out 54.880, 54.878 and 54.880 — the 0.002 is the browser rounding `height:245.44px` to 245.438, not the geometry. The fourth side is not a margin: the mark's right faces the query rather than an edge of the bar, so the 73.0 icon-to-text ink gap sits there instead, which is its own departure and unchanged. What follows: the mark's right edge is 515.41, `GMARK_RIGHT` has to match or the text would overlap it, and that puts the query on 582.8 and the bar on 2543px with the pan at 1635. The plus is untouched at 425/58/58 on a 2491px bar. Back: `left:386.1px;top:474.6px;width:121.9px;height:124.8px` with `GMARK_RIGHT = 508.0`. |

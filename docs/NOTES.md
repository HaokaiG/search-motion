...
| The bubble's second row is derived, not measured | The plate never shows a wrapped query, so nothing here comes off it. The width limit is 1438 — the bubble's right edge is pinned 241 from the frame's and the answer column starts on 241, so the widest the bubble can be is that column's span and at full width its left edge lands exactly on it. That is read off the layout, not off a string: an earlier version capped at 828, the reference query's own pill, which made every long query the same width and turned extra length into extra rows; by request the width now follows the text instead. A row is 54px, being the measured 102 less 24 of padding either side, which is also the body's line-height ratio (1.414) at 38.22 to within 0.05 — two independent derivations agreeing. The bubble grows DOWN from its measured top and `placeAnswer` moves the column by the same amount, so the one-row case is untouched: verified in exported pixels, 43 characters give 854-1679 by 102 with the answer's first ink on 469, 69 give 377-1679 by 102, and 95 give 241-1679 by 156 with the answer 54 lower. Back: `max-width:828px` for the old behaviour, or `height:102px` and `white-space:pre` and drop `placeAnswer` for none of it. |

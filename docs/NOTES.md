...
| The box's move waits for the press | By request. The supplied 350ms press is back at rest on f48.994 and the move used to start on f48 — a frame of overlap this build introduced, since the plate's own press is much shorter and clears long before. Only the move is held back; the face keeps lifting from f48, because that is the box changing colour rather than moving, and holding it too dropped f49 from 25.9 to 4.3. Back: `N2_MOVE0 = 48` and the exponent to 3.25. |
| The press starts 0.1s early | By request. The plate presses on f43; this starts on f40.6, so the first frames of the stretch land under the tail of the fill ramp while the button is still turning blue. Back: `N2_PRESS = 43`. |
...
| Caret 40.13px tall | Longer by request. The plate renders it 32, only ascender-to-baseline; the design file draws it 434.73..474.858, which is 40.128 — about 2px above the ascender and 5px below the baseline — so the longer number is the file's rather than invented. It measures 41px in the exported frame against text ink of 430..472. Back: 32px, `vertical-align:-4px`. |
...
| The prompt piece's bubble and box both follow the prompt's length | By request, and the rule is the search piece's, pointed at this piece. The bubble is pinned by its RIGHT edge and hugs its text — `AIM Chat_White.svg` draws it 861…1680 for its own string and the plate 969…1680 for this one, so the width is the text's and 1680 is the anchor, 240 in from the frame's. It grows leftward to at most 1439, where it meets the answer column at 241, a row is 99 (24 of padding either side of the body's own 51px line), and extra rows push the answer down by 51 each. Its corners come off the design file's path — 49.5 / 10 / 60 / 49.5, squaring the one nearest the sender — where a plain 45 pill had been assumed, since 45 is also half of 99 and the two are indistinguishable on one row. The box grows too: it keeps every inset it is drawn with, so a second row makes it one line taller (52.8, its own 1.2 leading), and it grows about the frame's CENTRE, because 426.01 + 1067.86/2 is 959.94 and 389.58 + 300.24/2 is 539.7 — growing from the top alone would walk it off that centre. Its three controls had to move from `top` to `bottom` insets to stay under the text. Two couplings are worth knowing. The bubble takes the BOX's line breaks rather than wrapping to its own width, because the box is this piece's field the way the panel is the other's — that is what keeps the morph continuous, the box's two rows becoming the bubble's two rather than the text reflowing inside three frames. The panel's prompt field is then made to agree with both, which is where the two pieces differ. The search piece's bar is one pill and cannot have rows, so nothing rendered there can define a break and the FIELD is the authority; here the box is on screen at full size, and forcing a 241px field's breaks onto a 960px box would end its first line at 42 characters where 45 fit, so the box keeps the authority and the field is sized to it. Same typeface both ends, so matching the width in EMS matches the break positions: 960/44 is 21.818em, which at 241px is an 11.05px field rather than the 12 the other controls wear — and Regular rather than the panel's Medium, since Medium's wider advances broke a 223-character prompt a whole row early. Derived from the field's own width, so a wider panel stays faithful, and the render never depends on the panel either way. Verified line for line, not just by row count: field, box and bubble agree on all 50 prefixes of a 48-word prompt, with the clock parked inside the morph before each edit. That last part matters, because measuring the LIVE `#nq` is what broke it first — the morph writes a font-size onto that element every frame, so a bubble measured off it broke a 68-character prompt twelve characters late whenever the previous render happened to land inside the morph. `n2PromptLines` states the box's resting 44/960 instead of reading them. And the query's leading now lerps 52.8 → 51 through the morph, so its rows land on the chip's; with that matched, `nq`'s landing is simply the chip's own 24px padding, where it used to be a 27.5 worked out for one row at the tighter leading. Both put the first glyph within half a pixel of y=318. The box is sized for the WHOLE prompt, not for however much has been typed — neither reference clip shows a box that changes size mid-type, and the resting rect is read by the morph, the halo and the export alike. Verified in exported pixels: one row draws 999…1680 at 99 tall, two draw 855…1680 at 150, the answer 51 lower, and the frame count is unchanged at 194. Back: `n2Fit(1)` pinned, `left:969px` on `.n2chip`, and `n2chip.textContent = N2_Q`. |
| The panel's prompt field opened as a sliver | Two bugs in the one expression, both in the auto-grow that makes a field hug its text. `applyN2` runs at boot, when the piece is still 1 and every `p2only` row is hidden — a hidden element measures 0, so `scrollHeight` came back 0 and `height:0px` was written and stuck until the first keystroke re-measured it. The field also never got its 11.05px scale, for the same reason: `n2FieldScale` bails on a zero width. `fitField` now refuses to size an element with no layout, and `applyPiece` calls `applyN2` rather than `n2Build` — the moment the panel becomes visible is the moment it can be measured. The second bug was silent and applied to BOTH pieces: `scrollHeight` carries the padding but not the border, and these are `border-box`, so handing it straight back left every field two pixels short of its own text. Measured: piece 1's query field 34 scroll against a 34 client where it was 32, the prompt field 33/33, 49/49 at two rows and 82/82 at four, and the height survives a switch to the other piece and back. |
| The contributors sidebar lags the data it is drawn from | Worth writing down because it has now looked unresolved three times. GitHub attributes a contributor from three places — a commit's author, its committer, and any `Co-Authored-By:` trailer in the message — and all three are clean here: 143 commits across both branches, every one authored and committed by `HaokaiG <HaokaiG@users.noreply.github.com>`, zero trailers, and no occurrence of the string in any message, tracked filename or tracked file. GitHub's own API agrees, including anonymous contributors: `curl -s "https://api.github.com/repos/HaokaiG/search-motion/contributors?per_page=100&anon=1"` returns one entry. The repository sidebar is a *different cache* from that API and rebuilds lazily — on a push to the default branch, or on its own within about a day — so it can still show a name the data no longer contains. Two things that look like the cause and are not. First, `git log --all` will happily show commits that only exist in `refs/original/*`, the backups a previous `git filter-branch` left behind; those are local, were never pushed, and finding trailers in them says nothing about the remote. Clear them with `git for-each-ref --format='delete %(refname)' refs/original \| git update-ref --stdin`, then expire the reflog and `git gc --prune=now`. Second, `git filter-branch -- --all` rewrites `refs/remotes/origin/*` too, so straight afterwards the remote-tracking refs describe the rewrite rather than the remote; `git fetch --prune` before believing them, or read `git ls-remote` instead. |
| A double hyphen in a markup comment breaks every exported frame | Not a design decision — a trap, and an expensive one to find because the symptom is nowhere near the cause. `stageSVG` serialises the live stage with `XMLSerializer` and hands the result to an `<img>` as a data URI, so **every HTML comment inside `#stage` or `#stage2` is copied into an XML document**. `--` is illegal inside an XML comment. Writing the name of a CSS custom property in one — `--ctx-bot` — made the parser reject the whole SVG, and the only symptom was `frame render failed` from `img.onerror`: no line number, no mention of comments, and the preview kept working perfectly because the DOM itself was fine. It only shows on export. The fix is to write such names without their leading hyphens in comments that live inside a stage; CSS comments in the `<style>` tag are safe, since that content is escaped as text rather than parsed as markup. To find it: `new DOMParser().parseFromString(stageSVG(),"application/xml").querySelector("parsererror")` names the line. |

...
| The camera follows the caret | **The baked `PAN` curve no longer drives the camera at all.** The pan chases a target derived from the newest character by a fixed fraction of the remaining distance each frame — a first-order lag, so it is smooth by construction and settles rather than stopping. `CAM_SPEED` scales that fraction and is exposed in the panel and as `#cam=`. The offset makes the landing exact: the target is caret − (`PARK_X` − the chip's run), so the last character puts it on `PAN_END`. The move is held until `CAM_START_S` (1.92s at 1x, stored as a fraction of the typing span so it re-times with the rate) — the plate holds its own camera 51 frames, and this is frame 46. Through the hold the target tracks the caret's advance **from the wake frame**, not its absolute position: pinning the absolute target at 0 and releasing it steps it straight to 1265 and the lag lurches after it at 129 px/frame. `CAM_LEAD = 2.4` and alpha 0.07 are then set to match the source's move rather than to park perfectly. Tracking that clip gives a peak of 54 px/frame at frame 66; these give **54 at 66**, with velocity ramping 14, 36, 45, 51, 42, 27, 17, 11, 7, 5, 3, 2, 1. **The trade:** buying the last of the parking instead (alpha .13, lead 1.8) lands within 2px but drags the peak out to frame 75, a third of a second late. As set, the pan is ~29px short of `PAN_END` as the outline lights and under 10 by the cut, on a move of 1583. At slow typing rates it has less room and falls further behind — 0.6x leaves ~80px at the cut — which the panel's readout shows; raising the camera speed closes it. `PAN`/`PAN_REF` are kept as the record and the way back. |

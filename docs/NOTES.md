...
| The G's margins are set by request, not measured | The mark runs 8.72% over the wide shot's 124.8 — 132.53 x 135.68, grown about its centre, holding the artwork's 250x256 aspect. Its margins are then specified rather than read: vertical 54.88, which is centring it in the 245.44 bar the G wears, and horizontal 19% over that at 65.31 on both sides. Measured back off computed style: 54.880 top, 54.878 bottom, 65.310 left and a 65.317 ink gap to the query — ratios 1.1901 and 1.1902. The plate uses none of it; its own left inset is 58.1 against 54.6 vertical, about 6% wider rather than 19%. The right-hand figure is the mark's own icon-to-text gap (`GMARK_TEXT_GAP`) replacing the 73.0 the plus still keeps, and it is an INK gap, so the text box starts 585.55 to put the 'h' — which bears 5.596 — 65.31 clear. What this does NOT touch is the text-to-chip gap, structurally: the chip hangs off wherever the text ends by `CHIP_GAP`, so moving the text moves the chip, measured 110.62 with either icon. Downstream the bar is 2546px and the pan 1638. Back: `left:386.1px` with `GMARK_RIGHT = 508.0` and `TEXT_LEFT_G` reading `ICON_TEXT_INK_GAP`. |

...
| Spark icon 15% over the plate | `.aichip svg.spark` at 77.05 against the measured 67x67 (ink at content x 2433…2500, y 504…571). By request. The chip is a fixed 455 with everything in it `flex:none`, so it eats slack rather than reflowing: 59 + 77.05 + 32 + ~239 leaves 47.7 spare and the label does not wrap. Back: 67. |

...
| The bar's outline sits **on** the bubble | The plate's glow is entirely outward — crossing inward from the stroke it reads 9, 6, 0 and stays 0 across the bar's face. This is the inverse: the outline is pulled in by half the core stroke so its outer edge lands on the bar's edge, and the bloom's mask is inverted to confine the glow to the pill. Profiled in an export, the core now reads 168 across y 420-424 with nothing at all outside, and the glow decays inward 63 → 6 by y 460. The chip's ring is deliberately the other way round, ringing the button from outside. Back: negate `BEAM_OFF` at the three uses in `layoutBar` and swap the two `fill`s in `#beamOnBar`. |

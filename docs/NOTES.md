...
| The camera is triggered by the text, eased by the plate | **The baked `PAN` curve no longer sets the timing, only the shape.** The move opens the frame the caret first reaches the middle of the frame (x=960), closes `CAM_LEAD` — 8% of the typing span — before the text lands, and in between runs `PAN`'s own normalised profile with its 51-frame hold stripped off, since the hold is now expressed by when the window opens. So the timing follows the typing at any rate while the easing stays the reference's: velocity reads 1, 63, **83**, 53, 32, 18, 9, 2, 0 across the move at 1x — accelerate to a peak, then decay — and it lands on `PAN_END` exactly. The plate instead holds 51 frames and ramps to a 56 px/frame peak on its own clock. Note a fast query makes a fast camera: at 2x the peak is 167 px/frame, and the shutter follows. `PAN`/`PAN_REF` are kept as the record and the way back. |
...
| Typing starts on frame 18, with the beam | The text's first character and the bar's gradient now arrive on the same frame, and the caret still gets two full blinks first — so the blink rate is **solved, not chosen**: `BLINK_HALF = (BEAM_F0 - BLINK_ON) / (2 * BLINK_CYCLES)` = 3. The plate blinks 8 on / 8 off and starts typing on 26 with the beam already running. Fitting two blinks into the twelve frames between the caret and the beam is what costs the speed — `BLINK_CYCLES = 1` would give a gentler 6 on / 6 off. Verified frame by frame: caret on 6-8, off 9-11, on 12-14, off 15-17, first character and `BEAM_FADE`'s first entry both on 18. Back: `BLINK_HALF = 8` and `TYPE_F0 = 26`. |

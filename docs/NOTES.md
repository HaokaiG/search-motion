...
| `CHIP_GAP_BG = 200` | Over footage only, against the measured 111. The bar is a scrim there and the chip only 75% opaque, so the two crowd each other at the plate's spacing. This one is **by eye — no measurement behind it**. It pushes the chip right, so the bar and pan grow with it; note the Medium query adds to that too, and the two together took the bar 2491 → 2637. Back: set it to 111. |

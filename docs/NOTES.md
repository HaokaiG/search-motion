...
| The camera tracks the caret | **The baked `PAN` curve no longer drives anything.** The pan is now whatever keeps the newest character on screen x = `TRACK_X`, so the move is driven by the text rather than the clock: it holds at 0 until the query is long enough to need it, then follows. `TRACK_X` is not chosen — it is `PARK_X` less the chip's gap, the chip and its inset, which is exactly where the caret sits once the bar has parked, so the follow lands on `PAN_END` with no seam between tracking and parking (verified: 0 at frames 0-38, 235/740/1174 at 50/70/90, and 1583 = `PAN_END` at 108). The plate instead holds 51 frames and then ramps to a 56 px/frame peak. The shutter follows for free, since σ is sampled off `panAt`. `PAN`/`PAN_REF` are kept as the measured record and the way back. |
| Typing starts on frame 38 | Two full caret blinks after it appears on 6, derived as `BLINK_ON + 2*(2*BLINK_HALF)` rather than written down. The plate starts on 26, mid-blink. Holding the start still is what lets the speed control move the end. Back: `TYPE_F0 = 26`. |

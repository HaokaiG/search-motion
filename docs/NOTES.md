...
| Act 2 lays no white card over footage | The reference cuts to white here — act 2's own `#fff` background is what covers the plate as the answer rises, and that is how the piece cuts out of the live shot. By request the card is gone over footage, so the plate carries on behind the answer. Verified in exported pixels rather than the preview: act 2's corner reads 12,63,55 on frames 140, 150 and 161, the same values act 1 reads on frame 60, and the white fraction goes from most of the frame to 0. Two consequences, both intended rather than worked around. The answer is dark ink on whatever the footage is, so a light or busy plate will fight it. And the query half blows out to whatever the mode puts behind act 2, which now makes three: white on the plain card (100% white), clear over transparency (100% clear), the plate over footage (0% white, corner reading the footage). Back: drop `#stage.has-bg #act2{background:transparent}`. |

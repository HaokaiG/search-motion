...
| The camera is triggered by the text, eased by the plate | **The baked `PAN` curve no longer sets the timing, only the shape.** The move opens `CAM_START_FRAC` of the way through the typing — 0.311, derived from wanting 1.92s at 1x — and closes on the frame the AI Mode outline first becomes visible (`RING_VISIBLE_F + TAIL`, read off `RING_FADE` rather than written down), and in between runs `PAN`'s own normalised profile with **both ends trimmed** — the 51-frame hold at the head, because the hold is expressed by when the window opens, and the asymptotic tail past `PAN_TAIL_CUT` at the end, because the close is meant to be the frame the camera stops. Untrimmed, the move was visibly still from ~105 with the window not closing until 115. Both ends re-time with the speed on their own — one is a character in the schedule, the other is the outline's own frame put back on the real clock — so nothing is computed from the rate. Verified across the range: it opens on 1.92s at 1x, 2.70s at 0.6x and 0.90s at 8x, always 0.311 through the sentence, and the pan lands on `PAN_END` at every one. The move runs slightly PAST the typing (108 → 114 at 1x), which is the tail of the plate's easing and nearly stopped by then. `CAM_EASE` then blends that curve 45% toward a quartic ease-**out** — the plate's sample is 1-frame resolution read back with a straight lerp, so on its own it carries small kinks and drops off at the trimmed end rather than settling. The blend target was smootherstep at first, which was wrong for this: its slow start fought the plate's hard ramp to a 56 px/frame peak, softening the one thing the reference does quickly. Quartic keeps the attack and still settles with zero velocity. Over its 69-frame window the peak is 48 px/frame reached 19% in, 84% of the move is done by halfway, and it is still moving on 114.8 before landing on `PAN_END`. Back: `CAM_EASE = 0`. `CAM_START_S` is the dial: opening it earlier lengthens the window and softens the peak, later shortens and sharpens it. `PAN`/`PAN_REF` are kept as the record and the way back. |
| Spark icon 9% over the plate, gap 31 | `.aichip svg.spark` at 73.2 against the measured 67x67 (ink at content x 2433…2500, y 504…571) — 15% up, then 5% back off it — and the chip's flex `gap` at 19 against the plate's 32. The 19 comes from a supplied crop where the gap reads about a quarter of the icon's width against the 0.42 this was sitting at — **an eyeball read of a pasted image, not a measurement off a frame**, so hold it loosely. Both by request. The chip is a fixed 455 with everything in it `flex:none`, so this eats slack rather than reflowing: 59 + 73.2 + 19 + 239.3 leaves 64.5 spare and the label does not wrap. Back: 67 and 32. |

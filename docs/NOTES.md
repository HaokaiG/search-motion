...
| The bubble's second row is derived, not measured | The plate never shows a wrapped query, so nothing here comes off it: the cap is the reference pill (822 measured, 824.7 as our own metrics render it, set at 828 — 3px of headroom, which is less than the ~8px the narrowest glyph at this size adds, so it cannot let a longer query through), and a row is 54px, being the measured 102 less 24 of padding either side. That 54 also happens to be the body's line-height ratio (1.414) at 38.22 to within 0.05, which is two independent derivations agreeing. The bubble grows DOWN from its measured top and `placeAnswer` moves the column by the same amount, so the one-row case is bit-for-bit what it was: verified in exported pixels, the bubble spans y289-391 at 102 tall with the answer's first ink on 469, and a wrapped one spans 289-445 at 156 with the ink on 523 — 54 lower, exactly. Back: `height:102px`, `white-space:pre`, and drop `placeAnswer`. |
